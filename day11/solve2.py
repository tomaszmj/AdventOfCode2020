...
class Seat:
    ...
    def new_label(self, occupied_neighbours: int) -> str:
        if self.label == EMPTY and occupied_neighbours == 0:
            return OCCUPIED
        if self.label == OCCUPIED and occupied_neighbours >= 5:
            return EMPTY
        return self.label
        ...
        for direction_x in [-1, 0, 1]:
            for direction_y in [-1, 0, 1]:
                if direction_x == 0 and direction_y == 0:
                    continue
                count += self.count_neighbours_in_direction(x, y, direction_x, direction_y)
        return count

    def count_neighbours_in_direction(self, x: int, y: int, dx: int, dy: int) -> int:
        x += dx
        y += dy
        while self.at(x, y).label == FLOOR:
            x += dx
            y += dy
        if self.at(x, y).label == OCCUPIED:
            return 1
        if self.at(x, y).label == EMPTY:
            return 0
