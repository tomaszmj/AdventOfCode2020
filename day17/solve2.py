...
# Space represents ACTIVE/INACTIVE points in infinite discrete 4-D Euclidean space:
...
class Space:
    def __init__(self):
        self.lower = Point(0, 0, 0, 0)
        self.upper = Point(-1, -1, -1, -1)  # space is from lower to upper (inclusive), upper < lower indicates 0 size
    ...
    def set_active(self, p: Point):
        self.lower = Point(min(p.x, self.lower.x), min(p.y, self.lower.y), min(p.z, self.lower.z), min(p.w, self.lower.w))
        self.upper = Point(max(p.x, self.upper.x), max(p.y, self.upper.y), max(p.z, self.upper.z), max(p.w, self.upper.w))
        self.actives.add(p)
    ...
    def yield_all_points_with_outliers(self) -> Iterable[Point]:
        for p in itertools.product(
                ...
                range(self.lower.w - 1, self.upper.w + 2, 1),
        ):
            yield Point(p[0], p[1], p[2], p[3])

    @staticmethod
    def yield_point_neighbours(p: Point) -> Iterable[Point]:
        for diff in itertools.product((-1, 0, 1), repeat=4):
            if diff != (0, 0, 0, 0):
                yield Point(p.x + diff[0], p.y + diff[1], p.z + diff[2], p.w + diff[3])
