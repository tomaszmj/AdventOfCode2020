...
class BagSet:
    ...
    def count_children(self, parent: str) -> int:
        count = 0
        for child_data in self.bags[parent]:
            count += child_data.amount * (1 + self.count_children(child_data.name))
        return count
...
def main():
    ...
    print(f"children of {checked_bag}: {bag_set.count_children(checked_bag)}")
